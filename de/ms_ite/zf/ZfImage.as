...
v1.93: added chris changes: filters for tiles, mousewheel				(20070427)
v1.92: dispatch a change event after parameter change					(20050711)
v1.91: after changing image, initially disallow clip-events				(20050817)
v1.90: added v2 event-model												(20050608)
v1.78: fixed redraw bug													(20050416)
v1.77: changed event-handling, using focus, changed base-class			(20050416)
v1.76: change clip-event handling, timer-handling cleaned				(20050414)
v1.75: mouse-wheel updates after timeout, label code removed			(20050327)
v1.74: added mouse-wheel support										(20050323)
v1.73: fixed clip/image buildup											(20050303)
v1.72: fixed rebuild/switch												(20050302)
v1.71: changed event-model												(20050301)
v1.70: clipping implemented, added clip-events							(20050225)
v1.61: callback registration changed									(20050225)
v1.60: convert to as2.0 class											(20050218)

v1.60:
modifications:
- can register multiple callbacks
- uses loader and custom load queue + handling

*/
/*
Copyright Zoomify, Inc., 2002-2003.  All rights reserved.

You may modify but not redistribute this source code file.  
This file may only be redistributed as a compiled SWF file with import protection enabled (see Macromedia Flash MX documentation).  

In all cases, modifications must be clearly called-out. This license agreement must be redistributed with the files whether 
modified or not, regardless of the purpose of redistribution.  Works distributed for commercial purposes must clearly include 
modifications and works within the scope of which the Zoomify technology is not the primary source of value.

See the file "Legal Notice.txt" included with this distribution for the complete Zoomify License Agreement
...
package de.ms_ite.zf {
	...
	public class ZfImage extends UIComponent {
...
*/	
		...
		private var gTileExtension:String;
...
//			trace( "upd "+mask_mc+" "+width+","+height);
	...
	*/				
				...
				if ( gCurrentTierBackground_mc != null) {
					gCurrentTierBackground_mc.scaleY = gCurrentTierBackground_mc.scaleX = gZoom * baseWidth / currentWidth;
				}
	...
	*/
	...
	//		trace( "pan: "+gLabelTier_mc.x+"\t"+gLabelTier_mc.y+"\t\t"+(gPanPositionX)+"\t"+(gPanPositionY));
		...
		public function toFront( layer:DisplayObject):void {
			gLabelTier_mc_labels.swapChildren( layer, gLabelTier_mc_labels.getChildAt( gLabelTier_mc_labels.numChildren - 1));
		}
...
*/			
	...
	*/
	...
	//		debug( "focus "+evt.type+": "+evt.target);
			hasKBFocus = true;
		}
...
//			debug("mousePan ("+this+")" + panEvent + " " + gInitialized + "," + gEnabled + ".");
	...
	*/
	...
	//		debug( "focusMan: "+getFocusManager());
...
//				mapLayer.removeChild( gLabelTier_mc);
				...
				debug( "## premakeLabelTier: "+gLabelTier_mc+".");
			}
			gLabelTier_mc.x = 0;
			gLabelTier_mc.y = 0;
	...
	//		gLabelTier_mc.height = gViewHeight;
			...
			if ( gLabelTier_mc_labels == null) {
				...
				debug( "## premakeLabelTier.labels: "+gLabelTier_mc_labels+".");
			}
...
*/	
	//		debug("##zfi tiers-");
		...
		protected function mhandler( evt:MouseEvent):void {
...
//				case MouseEvent.MOUSE_OUT:
		...
		protected function setupTiers():void {
			...
			for (var i:Number = 1; i < numPremadeLevels; i++) {
...
//				temp.quality = "BEST";
	...
	//		castStatusEvent("Image initialized.");
		...
		/*destroy: disposes of all memory used by the component.  This must be called whenever the 
			URL is changed. Can also be called to free up memory in applications where it is clear that 
			the viewing of an image is complete.*/
	...
	//		var current:Number = (new Date).getTime();
			...
			for (ctier = 1; ctier < numPremadeLevels; ctier++) {
...
*/			
	...
	*/						
	...
	//			delete gTilesUsedArray1;
	//			delete gTilesUsedArray2;
				gTilesUsedArray1 = new Array();
				gTilesUsedArray2 = new Array();
	...
	*/
					tempTile = gTilesUsedArray2[i];	//eval(gTilesUsedArray2[i]);
					tempTile.visible = false;
				}
				if (gCurrentTier > gBackgroundTier) {
	//				debug( "fv1+: "+gTilesUsedArray1.length);
	...
	*/
						tempTile = gTilesUsedOldTierArray2[i];		//eval(gTilesUsedOldTierArray2[i]);
						tempTile.visible = false;
					}
					if (gOldTier > gBackgroundTier) {
	//					debug( "fv2+: "+gCurrentTierOld_mc);
						fillViewWithParams(gOldTier, gCurrentTierOld_mc, gTilesUsedOldTierArray1);
	//					debug( "fv1+: "+gTilesUsedOldTierArray1.length);
					}
					for (i = 0; i < gTilesUsedOldTierArray2.length; i++) {
	/*					var consumed:Number = (new Date).getTime() - current;
						if ( consumed > CPU_LIMIT) {
	...
	*/
					tempTile = gTilesUsedArray1[i];		//eval(gTilesUsedArray1[i]);
	//				debug( "ivisiblize: "+tempTile.name);
					tempTile.visible = false;
				}
				if (gCurrentTier > gBackgroundTier) {
	//				debug( "fv1+: "+gCurrentTier_mc);
					fillViewWithParams(gCurrentTier, gCurrentTier_mc, gTilesUsedArray2);
	//				debug( "fv1+: "+gTilesUsedArray2.length);
				}
				for (i = 0; i < gTilesUsedArray1.length; i++) {
	/*				var consumed:Number = (new Date).getTime() - current;
					if ( consumed > CPU_LIMIT) {
	...
	*/
						tempTile = gTilesUsedOldTierArray1[i];		//eval(gTilesUsedOldTierArray1[i]);
	//					debug( "ivisiblize_old: "+tempTile.name);
						tempTile.visible = false;
					}
					if (gOldTier > gBackgroundTier) {
	//					debug( "fv1+: "+gCurrentTierOld_mc);
						fillViewWithParams(gOldTier, gCurrentTierOld_mc, gTilesUsedOldTierArray2);
	//					debug( "fv1+: "+gTilesUsedOldTierArray2.length);
					}
					for (i = 0; i < gTilesUsedOldTierArray1.length; i++) {
	/*					var consumed:Number = (new Date).getTime() - current;
						if ( consumed > CPU_LIMIT) {
	...
	*/
						tempTile = ZfTile( gTilesUsedOldTierArray1[i]);		//eval(gTilesUsedOldTierArray1[i]);
						if (tempTile != null && tempTile.visible == false && gOldTier != gBackgroundTier) {
	//						debug( " fV: tuoa1 "+tempTile.name);
	//						debug( "delete_old "+tempTile.name);
							tempTile.x = gOffscreenX;
							tempTile.cancel();
						}
					}
	...
	//			delete gTilesUsedArray1;
			...
			//If there are callbacks registered, call them now
	//		debug( "##fillv callbacks");
			
	//		castViewEvent( getX(), getY(), getZoom());
	//		castRectEvent( gScaledWindow);
			...
			if ( postEvents) {
				...
				o.y = getY();
				o.zoom = getZoom();
				o.labels = getLabelVisible();
				
				dispatchEvent( new ZfEvent( ZfEvent.VIEW));	//{ type:'zfview', target:this, detail:o});		
//				dispatchEvent( { type:'zfrect', target:this, detail:gScaledWindow});
			}
...
//					var mcTileString = "this.tier" + tier + "_mc" + "." + tileName;
					...
					if (tileSource != "missing") {
						...
						if (mcTile == null) {
	//						mClip.createClassObject( de.msite.ZfTile, tileName, tileLevel, {loader:loader});
							mcTile = new tileClass();	//mClip.createClassObject( tileClass, tileName, tileLevel, {loader:loader});
							mcTile.loader = loader;
							mClip.addChild( mcTile);
							//					createEmptyMovieClip(tileName, tileLevel);
							
							tileCache[tier][rowCounter][columnCounter] = mcTile;
//							mcTile = eval(mcTileString);
							mcTile.x = gOffscreenX;
							created = check;
							debug("CR** " +mClip.name+" / "+mcTile.name + "  ld_" + mcTile.isLoaded());
						}
	//					debug("LD** " + mcTile + "#" + mcTileString + " / ck_" + check + " / cr_" + created + "/  ld_" + mcTile.needsLoad());
						if ( !mcTile.isLoaded()) {		// || (mcTile.x == gOffscreenX && tileSource != "missing")) {
							//ms call 
	...
	//					preTierArray[preTierArray.length] = mcTile;
					}
				}
				if ( doBreak) break;
			}
	...
	*/
			...
			var propertiesURL:URLRequest = new URLRequest( _imagePath + "/ImageProperties.xml");
			xmlLoader
			...
			xmlLoader.load( propertiesURL);
		}
		
		protected function xmlLoaded( event:Event):void {
			var xml:XML = new XML( xmlLoader.data.toLowerCase());
			
			parseImagePropertiesData( xml);
		}
	
		public function setImageProperties( node:XML):void {
			_imagePath = node.firstChild.attributes.path;
			var pathLen:int = _imagePath.length;
			
			debug( "set imProps: "+_imagePath);
	/*		
			if (_imagePath.charAt(pathLen - 1) != '/') {
				_imagePath = _imagePath + "/";
			}
	*/
			...
			parseImagePropertiesData( node);
		}
		
		protected function parseImagePropertiesData( xml:XML):void {
	...
	//		var current:Number = (new Date).getTime();
			...
			debug( "props: "+xml);	
			imageWidth = parseInt( xml.@width);
			imageHeight = parseInt( xml.@height);
			tileSize = parseInt( xml.@tilesize);
			gVersion = parseInt( xml.@version);
			gTileExtension = xml.@extension;
			if ( gTileExtension.length <= 0) gTileExtension = 'via';

			debug("map loaded: " + imageWidth + "," + imageHeight + "pix ts" + gTileSize + " v" + gVersion + " / "+gTileExtension+".");
	...
	*/
	...
	//			debug( "tiles in tier( "+j+") : "+gTierTileCount[j]+".");
		...
		private function getFolderTileURL( tier:int, x:int, y:int):String {
			...
			var tilePath:String = _imagePath + "/TileGroup" + theCurrentOffsetChunk + "/" + tier + "-" + x + "-" + y + '.'+ gTileExtension;
