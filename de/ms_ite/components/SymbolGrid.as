package de.ms_ite.components {
	...
	import de.ms_ite.maptech.layers.SymbolLayer;
	...
	public class SymbolGrid extends VBox {
		
		public var layer:de.ms_ite.maptech.layers.SymbolLayer;
		public var table:DataGrid;
		public var _dataProvider:ListCollectionView = null;
		protected var symbolStyle:SymbolStyle;
		protected var dgS:DataGridColumn;		
		...
		protected var delRowIndex:int;
		
		protected var renameDialog:RenamePanel;
		
		protected var askDelAlert:Alert;
		
		public var hasItems:Boolean;
		
		public function SymbolGrid( st:SymbolStyle=null) {
			_dataProvider = null;
		...
		public function setupUI():void {
			...
			layer = new de.ms_ite.maptech.layers.SymbolLayer();
			layer.style = symbolStyle;
			layer.symbolClass = de.ms_ite.maptech.symbols.IconImageSymbol;
			layer.dataProvider = dataProvider;
			
			if ( _map != null) _map.addChild( layer);
			
	    	layer.addEventListener( ListEvent.ITEM_CLICK, handleSymbolClick);
	    	layer.addEventListener( Event.SELECT, handleSymbolSelect);
	    	...
	    	dgc.headerText = 'H';
	    	dgc.width = 25;
	    	dgc.dataField = 'hidden';
	    	var cbr:ClassFactory = new ClassFactory( mx.controls.CheckBox);
	    	cbr.properties = { enabled:false};
	    	dgc.itemRenderer = cbr;
	    	dgc.itemEditor = new ClassFactory( mx.controls.CheckBox);
	    	dgc.editorDataField = 'selected';
	    	columns.push( dgc);
	    	
	    	dgS = new DataGridColumn();
	    	dgS.sortCompareFunction = sortNumeric;
	    	dgS.width = 40;
	    	dgS.headerText = '#';
	    	dgS.dataField = 'sorting';	    	
	    	columns.push( dgS);
	    	
	    	dgc = new DataGridColumn();
	    	dgc.headerText = 'Title';
	    	dgc.dataField = 'company';	    	
	    	columns.push( dgc);
	    	
	    	dgc = new DataGridColumn();
	    	dgc.headerText = 'Adress';
	    	dgc.dataField = 'address';	    	
	    	columns.push( dgc);
			...
			ctxDelRow.addEventListener( ContextMenuEvent.MENU_ITEM_SELECT, askRowDel);
...
*/
			ctxZoomSheet = new ContextMenuItem( 'zoom to all');
...
//			table.editable = true;
		...
		private function sortNumeric( obj1:Object, obj2:Object):int {
			var value1:Number = ( typeof(obj1.sorting) == 'number') ? obj1.sorting : (( typeof(obj1.sorting) == 'string') ? new Number(obj1.sorting) : null);
			var value2:Number = ( typeof(obj2.sorting) == 'number') ? obj2.sorting : (( typeof(obj2.sorting) == 'string') ? new Number(obj2.sorting) : null);
			
			var res:int = 0;
			if (value1 < value2) res = -1;
			if (value1 > value2) res = 1;
			
//			debug( "??: "+dgS.sortDescending+" / "+value1+" / "+value2+" = "+res);
			return res;
		}
...
//        	debug( "dp change: "+evt);
        	...
        	if ( ctxAddCol != null) {
				ctxDelRow.enabled = ctxAddCol.enabled = temp;
        	}
//        	ctxClearSheet.enabled = 
        	if ( ctxZoomSheet != null) ctxZoomSheet.enabled = temp;
...
//	    	dispatchEvent( new Event( Event.CHANGE));
	    ...
	    protected function handleSymbolSelect( evt:Event):void {
	    	debug( "select");
	    	...
	    	var se:Event = new Event( Event.SELECT);
	    	dispatchEvent( evt);
	    }
...
//				ctxRenCol.enabled = headerClicked;
		...
		protected function askRowDel( evt:ContextMenuEvent):void {
			if ( dataProvider.length == 0) return;
			
			var row:Object = (evt.mouseTarget != null) ? evt.mouseTarget : ctxMouseTarget;

			if ( row is IListItemRenderer) {
				delRowIndex = table.itemRendererToIndex( row as IListItemRenderer);
				debug( "del: "+delRowIndex);
				
				askDelAlert = Alert.show( 'Sure to delete POI "'+dataProvider[ delRowIndex].company+'"?','Delete?', Alert.YES | Alert.NO, this, handleRowDel);
			}
		}

		protected function handleRowDel( evt:CloseEvent):void {
			
			if ( evt.detail != Alert.YES) return;
		...
		protected function addRow( index:int):void {
			if ( dataProvider.length == 0) dataProvider.addItem( {company:'New', location:''});
			else {
				var item:Object = cloneEmpty( dataProvider.getItemAt(0));
				item.catid = dataProvider.getItemAt(0).catid;
				item.company = 'New';
		...
		protected function delRow( index:int):void {
			debug( "deleting: "+index+" / "+dataProvider.length);
			if ( 0 <= index && index < dataProvider.length) {
				var tse:TableSheetEvent = new TableSheetEvent( TableSheetEvent.TS_DELETE_ITEM);
				tse.item = dataProvider[ index];
				tse.index = index;
			...
			debug( "deleted: "+index+" / "+dataProvider.length);
		}	
